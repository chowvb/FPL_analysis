...
def plot_octagon(ax, center, size, team_one, team_two):
    ...
    # Plot the octagon
    ax.fill(x, y, color = "Black", alpha = 0.5)
    ax.plot(x2, y2, color = "Red", alpha = 0.3)
    ax.plot(x3, y3, color = "Blue", alpha = 0.3)
...
def get_team_strength(t1, t2):
    t1_scores = team_strength[team_strength["short_name"] == t1]
    t2_scores = team_strength[team_strength["short_name"] == t2]
...
t1, t2 =  get_team_strength("ARS", "LIV")


# Call the function to plot the octagon
plot_octagon(ax, center=(0, 0), size=1400, team_one= t1, team_two= t2 )

# Set axis labels and title
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_title('Octagon Plot')
